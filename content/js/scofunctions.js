...
function loadPage()
{
   ...
   // var result = pipwerks.SCORM.connection.initialize();
   var result = doLMSInitialize();

   // var status = pipwerks.SCORM.data.get("cmi.completion_status");
   var status = doLMSGetValue( "cmi.completion_status" );

   if (status == "not attempted" || status == "" || status =="unknown")
   {
	  // the student is now attempting the lesson
     // pipwerks.SCORM.data.set("cmi.completion_status", "incomplete");
   ...
   exitPageStatus = false;
	 bookmark = doLMSGetValue("cmi.core.lesson_location", false);
	 suspendData = doLMSGetValue("cmi.suspend_data", false);
	// init(bookmark, suspendData)
...
function computeTime()
{
   ...
   // pipwerks.SCORM.data.set("cmi.session_time", formattedTime );
   doLMSSetValue( "cmi.session_time", formattedTime );
}
...
function doBack()
{
   ...
   // result = pipwerks.SCORM.connection.terminate();
   //result = doLMSFinish();
...
function doContinue( status )
{
   ...
   // pipwerks.SCORM.data.set( "cmi.exit", "" );
   doLMSSetValue( "cmi.exit", "" );

   // var mode = pipwerks.SCORM.data.get( "cmi.mode" );
   var mode = doLMSGetValue( "cmi.mode" );

   if ( mode != "review"  &&  mode != "browse" )
   { 
      // pipwerks.SCORM.data.set( "cmi.completion_status", status );
